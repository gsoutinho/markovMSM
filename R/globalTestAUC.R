...
#' res<-globalTestAUC(db_long, db_wide, from=1, to=3, replicas = 20, tmat=tmat)
...
#' data(prothr)
#' 
#' resGT1<-globalTestAUC(db_long = prothr, db_wide = NULL, from=1, to=3, replicas=20, limit=0.90, 
#'         quantiles=c(.05, .10, .20, .30, 0.40))
#'         
#' round(resGT1$localTests,4)
...
#' @importFrom "stats" pchisq pnorm quantile sd na.omit terms
...
globalTestAUC<- function(db_long, db_wide, from=1, to=3, quantiles=c(.05, .10, .20, .30, 0.40), 
    ...
    if(!is.null(db_wide)){
      ...
      if(length(namesStates)==3){
        ...
        if(i==1){
          ...
          valToQuantiles<-valToQuantiles[valToQuantiles!=0] #para retirar valores zeros!
