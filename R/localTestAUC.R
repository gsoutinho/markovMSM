...
#' 
#' 
localTestAUC<- function(db_long, db_wide, times, from=1, to=3, tmat = NULL,
  ...
  for(h in 1:length(times)){
    ...
    for(k in 1:M){
      ...
      #resAJf<-resAJ[[1]][resAJ[[1]][,1] %in% tComuns,] #estava por defeito 1!!!!
  ...
  res<-list(localTest=p.valueAll.f, trans = tmat, times=times, DIF=DIF, AJall=AJall, LMAJall=LMAJall, from=from, 
            to=to, ET.qiAll=ET.qiAll, ET.qi2All=ET.qi2All, ET.qsAll=ET.qsAll, ET.qs2All=ET.qs2All, replicas=M, limit=limit)
